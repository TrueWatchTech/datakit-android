    ...
    public static void startApp(Application app) {
        try {
            ...
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                @Override
                public void run() {
                    FTAppStartCounter.get().appOnCreateCompleted(System.nanoTime());
                }
            });
        } catch (Exception e) {
...
//        }
    ...
    /**
     * Gradle plug-in hook used to replace
     * {@link OkHttpClient#newWebSocket(Request, WebSocketListener)} calls.
     *
     * <p>Applications should not call this method directly. The {@code ft-plugin} generated bytecode
     * calls it, so the plug-in and Agent versions must agree on this method signature. When RUM or
     * automatic Resource collection is disabled, or the URL is filtered, the call is forwarded to
     * the original factory without collecting a Resource.</p>
     *
     * <p>Collection covers only the WebSocket handshake. The original listener continues to receive
     * all callbacks, and connection messages and lifetime are not collected.</p>
     *
     * @param factory WebSocket factory that starts the connection
     * @param request WebSocket handshake request
     * @param listener original application listener
     * @return WebSocket returned by the original factory
     */
    public static WebSocket trackOkHttpNewWebSocket(WebSocket.Factory factory,
                                                    Request request,
